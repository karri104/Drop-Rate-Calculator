...
def main():
    ...
    if k == 0:
        ...
        print(f"The chance of getting 0 drops is {chances[1]:.{sig_digs}f}%.")

        #Chance to get more than 0 drops
        print(f"The chance of getting more than 0 drops is {chances[2]:.{sig_digs}f}%.")
    else:
        #Chance to get less than x drops
        print(f"Chance to get less than {k} drops is {chances[0]:.{sig_digs}f}%.")

        #Chance to get exactly x drops
        print(f"Chance to get exactly {k} drops is {chances[1]:.{sig_digs}f}%.")

        #Chance to get more than x drops
        print(f"Chance to get more than {k} drops is {chances[2]:.{sig_digs}f}%.")

        #Expected amount of drops
        print(f"Expected amount of drops is {n * (prob[0] / prob[1]):.{sig_digs}f}.")
